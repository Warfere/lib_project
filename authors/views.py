...
class GetAuthors(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer=AuthorSerializer):
        ...
        for author in authors:
            if self.request.POST.get('name', "").lower() == author['name'].lower() and self.request.POST.get('last_name', "").lower() == author['last_name'].lower():
                raise exceptions.ValidationError('Name and last name must be unique', code=400)
        serializer.save()
